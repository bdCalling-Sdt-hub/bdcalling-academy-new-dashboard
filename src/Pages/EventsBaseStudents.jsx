...
const EventsBaseStudents = () => {
    ...
    const [requestingStudents, Students, StudentsError, refetch, isError] = useGetRequest('eventBasedStudent', `/students?student_type=event&page=${page}${filterBy?.number && `&phone_number=${filterBy?.number}`}${filterBy?.name && `&name=${filterBy?.name}`}${filterBy?.category && `&event_name=${filterBy?.category}`}${filterBy?.dob && `&dob=${filterBy?.dob}`}`)//phone_number=01317659523&name=r&category_name=1&
    ...
    // add student
    const onSubmitStudent = (values) => {//eventName
        const data = {
        ...
        <>
            <div className='grid-2 '>
                <div className='w-full'>
                    <PageHeading text={`All Events Base Students.`} />
                </div>
            ...
            <div id='allStudent' className='bg-[var(--third-color)] my-8 rounded-md '>
                <h3 className='section-title px-5'>All Events Register Base Students List</h3>
                {
                    (filterBy?.name || filterBy?.number || filterBy?.category || filterBy?.dob) && <div className='flex justify-start items-center gap-2 mb-2 -mt-3 ml-5'>Filter by
                        ...
                        {filterBy?.category && <> <strong>Event </strong> : {filterBy?.category} </>}
...
export default EventsBaseStudents
