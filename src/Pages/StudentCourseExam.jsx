        ...
        <>
            ...
            <Form onFinish={onFinish} layout="vertical">
                {questions.slice(current * 4, current * 4 + 4).map((q, index) => (
                    <Form.Item
                        key={index}
                        label={<span className='text-lg font-medium'><strong>{current * 4 + index + 1} . </strong> {q.question}</span>}
                        name={`question-${index}`}
                        rules={[{ required: true, message: 'Please select an option!' }]}
                    >
                        <Radio.Group className='flex justify-start items-start gap-2 flex-col'>
                            {q.options.map((option, idx) => (
                                <Radio className='text-base' key={idx} value={option}>{option}</Radio>
                            ))}
                        </Radio.Group>
                    </Form.Item>
                ))}
                <div className='flex justify-center items-center gap-2'>
                    ...
                    {
                        current === (Math.ceil(questions?.length / 4) - 1) && <button disabled={current !== (Math.ceil(questions?.length / 4) - 1)} className='flex justify-center items-center gap-2 w-44 border-blue-500 rounded-md py-3 border bg-blue-500 text-white disabled:bg-gray-400 disabled:text-black disabled:border-gray-400'> Complete exam</button>
                    }
                </div>
            </Form>
            <Modal
                ...
                centered
            >
                <div className='flex justify-center items-center flex-col gap-4 my-2 p-2'>
                    <p className='text-base font-semibold text-[#000000]'>Dear student Your result is</p>
                    <p className='text-2xl'>6/10</p>
                    <div className='flex justify-center items-center gap-2'>
                        <button className='flex justify-center items-center gap-2 w-44 border-blue-500 rounded-md py-3 border bg-blue-500 text-white disabled:bg-gray-400 disabled:text-black disabled:border-gray-400'> Back to course video</button>
                        <button type='button'  className='flex justify-center items-center gap-2 w-44 border-blue-500 rounded-md py-3 border text-blue-500 disabled:bg-gray-400 disabled:text-black disabled:border-gray-400'> Continue</button>
                    </div>
                </div>
            </Modal>
        </>
