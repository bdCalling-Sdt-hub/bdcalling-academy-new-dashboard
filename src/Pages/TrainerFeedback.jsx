                    ...
                    reviews?.data?.data?.map(item => {
                        return <div className='card-shadow p-4 rounded-md' key={item?.id}>
                            <div className='flex justify-start items-center gap-2'>
                                {
                                    item?.user?.image ? <img className='h-10 w-10 rounded-full' src={`${item?.user?.image}  `} alt="image" /> : <img className='h-10 w-10 rounded-full' src= "https://i.ibb.co/1GH871B/blank-profile-picture-973460-960-720.webp" alt="image" />
                                }
                                
                                <p>{item?.user?.name}</p>
                            </div>
                            <p className='tracking-wide leading-7 mt-2 text-justify'>{item?.feedback}</p>
                        </div>
                    })
                }
            </div>
